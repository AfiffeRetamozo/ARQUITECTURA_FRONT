...
export class CarreraListComponent implements OnInit {
  ...
  mostrarCarrerasAbiertas: boolean
  textoBotonFiltro: string
  ...
  ngOnInit() {
    if (!parseInt(this.router.snapshot.params.userId) || this.router.snapshot.params.userToken === " ") {
      ...
      this.textoBotonFiltro = "Filtrar Activas"
      this.mostrarCarrerasAbiertas = false
    }
  }
  ...
  getCarrerasAbiertas(): void {
    this.carreraService.getCarrerasAbiertas(this.userId, this.token)
      .subscribe(carreras => {
        this.carreras = carreras
        this.mostrarCarreras = carreras
        if (carreras.length > 0) {
  ...
  onClickFilterButton() {
    if (this.mostrarCarrerasAbiertas) {
      this.getCarreras()
    } else {
      this.getCarrerasAbiertas()
    }

    this.mostrarCarrerasAbiertas = !this.mostrarCarrerasAbiertas 
    this.textoBotonFiltro = this.mostrarCarrerasAbiertas ? "Mostrar todas" : "Filtrar Activas" ;
