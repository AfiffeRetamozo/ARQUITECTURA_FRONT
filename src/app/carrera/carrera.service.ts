...
export class CarreraService {
  ...
  getCarrerasAbiertas(usuario: number, token: string): Observable<Carrera[]> {
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${token}`
    })
    return this.http.get<Carrera[]>(`${this.backUrl}/carrera/${usuario}/disponible`, { headers: headers })
  }
